...
Este documento define la estrategia de alto nivel dividida en 4 fases de ingeniería. Para el detalle técnico de tareas de micro-gerencia, consultar `.specify/backlog.md`.
...
- [x] **Integración de Spec Kit**: Formalización de Spec-Driven Development e inicialización de la CLI `.specify/` en el proyecto.
...
### v0.14.4 - Expiración de Tareas & Reporte de Circuitos (Junio 2026)

- **Resolución de Limbo:** Corregir filtro de expiración en `cleanupExpiredTasks` para considerar estados de inicialización y estabilización, liberando tareas atrapadas en "En espera".
- **Resumen Hídrico Multi-Circuito:** Implementar reporte por circuito en doble línea (Foger/Irrigation/Soil_Wet y Fertirrigation/Fumigation) en el log de telemetría diaria, resolviendo reportes omitidos y ceros falsos.
- **Normalización de Hora & Responsividad:** Estandarizar sufijo de 12 horas a `am`/`pm` sin puntos ni espacios en el frontend, simplificar fecha de 'Ayer' y rediseñar responsivamente las tuplas del timeline en el panel de administración.
