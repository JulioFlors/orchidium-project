  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const currentTask = await tx.taskLog.findUnique({
        where: { id: taskId },
        select: { status: true, actualStartAt: true, duration: true },
      })
      ...
      // Calcular tiempo efectivo y restante (solo si estaba IN_PROGRESS con inicio real)
      let timeInfo = ''
      ...
      // Si estaba en IN_PROGRESS, generamos nota enriquecida con tiempos de operación.
      const finalNotes =
        notes ||
