    ...
    if (activeAudit === 'nvs') {
      ...
      if (isChartable) {
        setDisplayPayload((prev: unknown) => {
          ...
          // Si el payload es un snapshot escalar (ej: { lux: 123 }), lo convertimos en un punto histórico
          if (
            incomingHistory.length === 0 &&
            activeAudit &&
            incomingPayload[activeAudit] !== undefined
          ) {
            // Generar timestamp si no viene en el payload (Auditorías son streaming en vivo)
            ...
            incomingHistory = [[timestamp, incomingPayload[activeAudit]]]
          }
