...
import { Card, DeviceViewHeader } from '@/components'
        ...
        if (json.snapshots && json.snapshots.length > 0) {
          ...
          if (isChartable) {
            ...
            for (const snap of json.snapshots) {
              ...
              const createdAt = new Date(snap.createdAt).getTime() / 1000

              if (snapData?.history && Array.isArray(snapData.history)) {
                allHistory.push(...snapData.history)
              } else if (snapData !== undefined && snapData !== null) {
                // Si es un snapshot escalar (ej: { value: 123 } o { lux: 123 })
                const val = snapData.value ?? snapData[category]

                if (val !== undefined) {
                  allHistory.push([createdAt, val])
                }
              }
            }
