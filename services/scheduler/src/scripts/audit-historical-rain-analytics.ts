...
async function main() {
  ...
  const categoryConsolidated: any[] = []
  const categoryDetectedExact: any[] = []
  const categoryMissedShort: any[] = [] // Garúas insignificantes (< 15 min)
  const categoryMissedSignificant: any[] = [] // Lluvias de duración significativa (>= 15 min) no detectadas
  ...
  for (const obs of observedEvents) {
    ...
    const matches = inferedEvents.filter((inf) => {
      ...
      const overlaps = Math.max(0, Math.min(obsEnd + TOLERANCE_MS, infEnd + TOLERANCE_MS) - Math.max(obsStart - TOLERANCE_MS, infStart - TOLERANCE_MS))
      return overlaps > 0
    })
    ...
    if (matches.length > 0) {
      ...
      const infStartStr = primaryMatch.startedAt.toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Caracas' })
      const infEndStr = primaryMatch.endedAt ? primaryMatch.endedAt.toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Caracas' }) : 'En curso'
  ...
  console.log('--------------------------------------------------------------------')
  console.log(`✅ 1. EVENTOS DETECTADOS CON ÉXITO: ${categoryDetectedExact.length + categoryConsolidated.length}`)
  console.log('--------------------------------------------------------------------')
  
  console.log('\n--- 🎯 Coincidencias Directas / Exactas ---')
  for (const item of categoryDetectedExact) {
    const obsS = new Date(item.obs.startedAt).toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Caracas' })
    const obsE = new Date(item.obs.endedAt).toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Caracas' })
    console.log(`- [${item.obs.dayOfWeek} ${item.obs.id}] Real: ${obsS}-${obsE} (${item.obsDurMin.toFixed(0)}m) => Inferido: ${item.infStartStr}-${item.infEndStr}`)
  ...
  if (categoryConsolidated.length > 0) {
    ...
    for (const item of categoryConsolidated) {
      console.log(`- [${item.obs.dayOfWeek} ${item.obs.id}] Real: ${item.obs.startedAt.substring(11,16)} (${item.obsDurMin.toFixed(0)}m) => ${item.note}`)
      console.log(`  └─ Descripción: "${item.obs.description}"`)
    }
  }

  console.log('\n--------------------------------------------------------------------')
  console.log(`⚠️ 2. EVENTOS NO DETECTADOS POR EL MOTOR (FALSOS NEGATIVOS): ${categoryMissedShort.length + categoryMissedSignificant.length}`)
  console.log('--------------------------------------------------------------------')
  ...
  for (const item of categoryMissedSignificant) {
    const obsS = new Date(item.obs.startedAt).toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Caracas' })
    const obsE = new Date(item.obs.endedAt).toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Caracas' })
    console.log(`- [${item.obs.dayOfWeek} ${item.obs.id}] [${obsS} - ${obsE}] (${item.obsDurMin.toFixed(0)} min): "${item.obs.description}"`)
  }
