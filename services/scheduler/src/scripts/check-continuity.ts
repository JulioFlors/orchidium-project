...
async function main() {
  console.log(
    'Verificando continuidad de datos en InfluxDB del 10 de Julio en la madrugada (03:50 UTC a 05:00 UTC)...',
  )
  ...
  if (rows.length > 0) {
    console.log(
      `Primera muestra: ${rowTimeToDate(rows[0].time).toLocaleString('es-VE', { timeZone: 'America/Caracas' })}`,
    )
    console.log(
      `Última muestra: ${rowTimeToDate(rows[rows.length - 1].time).toLocaleString('es-VE', { timeZone: 'America/Caracas' })}`,
    )
  }
}
