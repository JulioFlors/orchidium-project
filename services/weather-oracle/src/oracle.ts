...
export async function syncOpenWeatherMap() {
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (status === 401) {
        Logger.warn(
          'OpenWeatherMap: API Key inválida o no activada aún (esperar 2h-24h tras registro).',
        )
      } else {
        Logger.error(
          `Fallo al sincronizar con OpenWeatherMap: [HTTP ${status}] ${error.message}`,
          error,
        )
      }
    } else {
      Logger.error(`Fallo al sincronizar con OpenWeatherMap:`, error)
    }
...
export async function syncAgroMonitoring() {
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      Logger.error(
        `Fallo al sincronizar suelo con AgroMonitoring: [HTTP ${error.response?.status}]`,
        error,
      )
    } else {
      Logger.error('Fallo al sincronizar suelo con AgroMonitoring:', error)
    }
...
export async function syncAgroHistory() {
  ...
  try {
    ...
    if (axios.isAxiosError(error) && error.response?.status === 401) {
      Logger.warn(
        'AgroMonitoring History (401): Acceso denegado. Esta función (Historial de Precipitación) suele requerir un plan de pago (Premium) en AgroMonitoring.',
      )

      return false
    }

    Logger.error('Fallo al sincronizar historial con AgroMonitoring:', error)
